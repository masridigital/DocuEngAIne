...
| Company related items | `ResourceLink` (FromType/FromId ↔ ToType/ToId, optional Label). Types: Company, Asset, Document, Runbook, KeeperLink. Unique per tenant pair. Both ends `ForTenant` or 400. `GET/POST/DELETE /api/links`. Company detail includes `relatedLinks` count + short list. `GET /api/companies/{id}/graph` returns nodes + edges for that company. Not Hudu tabs. `AssetDocumentLink` stays the asset↔doc convenience. | Phase2C | `/companies/:id` Related + Relationships. Other-tenant company graph = 404. Other-tenant entity = 400. |
